...
class Upload
{
    ...
    public static function Exists($file)
    {
        ...
        if ($_FILES[$file]['error'] !== 0) {
            Log::err('There was an error in file upload: `' . 
                        self::$FileMessages[$_FILES[$file]['error']] . '`.');
            return false;
        }
    ...
    public static function IsAllowed($file, $extension, $primitive=true)
    {
        ...
        if ($primitive) {
            ...
            if (!in_array($fileExt, $extension)) {
                Log::war("Invalid extension actual: `{$fileExt}`, allowed: `".
                            implode(', ', $extension).'`.');
                return false;
            }
            else {
                Log::inf("Extension is valid: `{$fileExt}`, allowed list: `" . implode(', ', $extension) . "`.");
                return true;
            }
        }
        ...
        if ($mime = self::MimeType($file, $primitive)) {
            ...
            Log::war("Extension isn't allowed; determinant by mime: `{$mime}`, actual: `".
                        implode(', ', $extByMime) . "`, allowed: `".implode(',', $extension)."`.");
            return false;
        }
        else {
    ...
    public static function MimeType($file, $simple=true)
    {
        ...
        if ($typeHead != $typeInfo) {
            Log::war('On checking file type there\'s mismatch on what we got from PHP function and http header;
                     finfo_file: `' . $typeInfo . '`, http header: `' . $typeHead . '`.');
        }
    ...
    private static function process($file)
    {
        ...
        Log::inf('Uploading file: ' . print_r($File, true));
        ...
        # Is Error ?
        if ($fileError === 0) {
            Log::inf('File message: "' . self::$FileMessages[$fileError] . '"');
        }
        elseif ($fileError === 4) {
            Log::inf('File upload warning: "' . self::$FileMessages[$fileError] . '"');
            return array('error' => 'File upload warning: "' . self::$FileMessages[$fileError] . '"');
        }
        else {
            Log::err('File upload error: "' . self::$FileMessages[$fileError] . '"');
