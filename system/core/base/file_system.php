...
class FileSystem_Base
{
    ...
    public static function Rename($oldName, $newName, $directory=false)
    {
        ...
        if (rename($oldName, $newName)) {
            Log::inf("File was renamed from: `{$oldName}`, to `{$newName}`.");
            return true;
        }
        else {
            Log::err("Error while renaming file: `{$oldName}`, to `{$newName}`.");
            return false;
        }
    }
    //-
...
class FileSystemSelect
{
    ...
    {
        if (!file_exists($path)) {
            Log::war("Selecting directory or file which doesn't exists!");
            return false;
        }
        else {
            Log::inf("Making new selection: `{$path}`.");
            $this->path = $path;
    ...
    public function rename($newFilename, $autoOnExist=true, $keepExtention=true)
    {
        ...
        if (file_exists(ds($pathOnly.'/'.$newFilename)))
        {
            if ($autoOnExist) {
                ...
                Log::war("File aready exists: `" . ds($pathOnly.'/'.$newFilename) . '`.');
                return false;
            }
        }
