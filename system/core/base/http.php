...
class HTTP_Base
{
    ...
    private static function isAllowed($url, $force=false)
    {
        if (!Cfg::get('system/allow_redirects', true) && !$force) {
            Log::war("Redirects to `{$url}` failed. Redirects aren't allowed in your config!");
            return false;
        }
        else {
            if ($force) {
                Log::inf("Redirecting to `{$url}` by force. Redirect isn't allowed in your config, but here it was called by force!");
            }
            return true;
        }
    }
