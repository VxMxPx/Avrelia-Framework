...
class Plug_Base
{
    ...
    public static function Enable($plug)
    {
        ...
        if (method_exists($className, '_OnEnable')) {
            ...
            Log::inf("Method `_OnEnable` not found in `{$className}`.");
            $return = true;
        }
    ...
    public static function Disable($plug)
    {
        ...
        if (method_exists($className, '_OnDisable')) {
            ...
            Log::inf("Method `_OnDisable` no found in `{$className}`.");
            return true;
        }
    }
    //-
    ...
    public static function SetPublic($fullPath)
    {
        ...
        if (Cfg::get('plug/debug') && is_dir($fullPublicPath)) {
            Log::inf("The debug mode is enabled, will remove folder: `{$fullPublicPath}`.");
            FileSystem::Remove($fullPublicPath);
        }

        # Exists not? :)
        if (!is_dir($fullPublicPath)) {
            Log::inf("Can't find public copy, creating it: `{$fullPublicPath}` from `{$fullComPublicPath}`.");
    ...
    public static function GetConfig($fullPath)
    {
        ...
        Log::inf("The following config files were included: \n" . implode("\n", $included));
    ...
    public static function GetDriver($fullPath, $type, $construct=true, $prefix=false)
    {
        ...
        # Resolve filenames
        # 1. interface
        $interfaceClass  = 'c' . to_camelcase($plugName) . 'Driver';
        $interfaceClass .= ($prefix) ? to_camelcase($prefix) : '';
        ...
        # 2. base
        $baseClass  = 'c' . to_camelcase($plugName) . 'Driver';
        $baseClass .= ($prefix) ? to_camelcase($prefix) : '';
        $baseClass .= 'Base';
        ...
        # 3. driver
        $driverClass  = 'c' . to_camelcase($plugName) . 'Driver';
        $driverClass .= ($prefix) ? to_camelcase($prefix) : '';
        $driverClass .= to_camelcase($type);
        ...
        if (!class_exists($driverClass, false)) {
            if (file_exists($driverFile)) {
                ...
                Log::war("Can't load driver: `{$driverClass}` from `{$driverFile}`, file not found.");
                return false;
            }
        }

        if (!class_exists($driverClass, false)) {
            Log::war("Class `{$driverClass}` not found in `{$driverFile}`.");
            return false;
        }

        Log::inf("Driver was loaded: `{$driverClass}`.");
        if ($construct) {
        ...
        foreach ($Components as $component)
        {
            ...
            if (!$className) {
                ...
                if (file_exists($fullClassFileName)) {
                    ...
                    Log::war("Can't find plug: `{$fullClassFileName}`.");
                    $Failed[] = $component;
                    if ($stopOnFailed) {
                        break;
                    }
            ...
            if ($autoInit)
            {
                ...
                if (method_exists($className, '_OnInit'))
                {
                    if (!$className::_OnInit()) {
                        Log::war("Method: `_OnInit` in `{$className}` failed!");
                    }
                }
            }
        }
    ...
    {
        # Guess class name :)
        $classNameOne = 'c' . to_camelcase($plug, true);
